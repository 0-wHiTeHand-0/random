...
while ( $nombre = <NOMBRES>) {
	...
	$url='http://blancas.paginasamarillas.es/jsp/resultados.jsp?no='.$temp.'&sec=XX&lo=XXXXXXXX&calle=XXXXXXX&numero=XXXXXXX&pgpv=0&tbus=0&nomprov=XXXXXXXX&idioma=tml_lang';
	$contenido = get($url);
	sleep 2;
